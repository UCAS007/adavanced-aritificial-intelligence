...
if(os.path.exists('validate.pkl')):
    fin=open('validate.pkl','r')
    jointext=pickle.load(fin)
    fin.close()
else:
    jointext=train.getValidateTextList()
    fout=open('validate.pkl','w')
    pickle.dump(jointext,fout)
    fout.close()

# jointext have use jieba
...
fin=open('pipeline.pkl','r')
pipeline=pickle.load(fout)
